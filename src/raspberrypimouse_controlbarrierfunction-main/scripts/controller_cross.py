...
def si_barrier_cert(dxi, x, obstacles, barrier_gain=80, safety_radius=0.3, magnitude_limit=0.25):
    ...
    for i in range(N):
        for k in range(obstacles.shape[0]):
            ...
            hObs = np.sum(errorObs**2) - (obstacle_radius + safety_radius)**2
    ...
    result = solvers.qp(H, matrix(f), matrix(A), matrix(b))['x']
    end_time = time.time()
