...
def callSolve(Ox,Oy,Px,Py):
    ...
    def func(x):
        return ((((Px-Ox)*x))/1)**4 + ((((Py-Oy)*x))/2)**4 - 1
...
def si_position_controller_2(xi, positions, x_velocity_gain=1, y_velocity_gain=1, velocity_magnitude_limit=0.5):
    _,N = np.shape(xi)
    dxi = np.zeros((2, N))
...
def si_barrier_cert(dxi, x, obstacles, barrier_gain=80, safety_radius=0.2, magnitude_limit=0.5):
    ...
    for i in range(N):
        for k in range(obstacles.shape[0]):
            errorObs = x[:, i] - obstacles[k, :2]
            # obstacle_radius = obstacles[k, 2]
            obstacle_radius = callSolve(obstacles[k, :2][0], obstacles[k, :2][1], x[:, i][0], x[:, i][1])
            # print(obstacle_radius)
            # hObs = superellipse_barrier(errorObs, obstacle_radius + safety_radius)
            # print(sqrt(obstacle_radius[0]**2 + obstacle_radius[1]))
            # print(sqrt(errorObs[0]**2 + errorObs[1]**2))
            hObs = sqrt(errorObs[0]**2 + errorObs[1]**2) - sqrt(obstacle_radius[0]**2 + obstacle_radius[1]**2)
            
            Aob[countObstacles, 2*i:2*i+2] = -2*errorObs
            bob[countObstacles] = barrier_gain * pow(hObs, 3)
            countObstacles += 1
