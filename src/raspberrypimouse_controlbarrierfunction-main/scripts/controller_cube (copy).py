...
#     return dxi
def si_position_controller(xi, positions, x_velocity_gain=0.2, y_velocity_gain=0.2, velocity_magnitude_limit=0.5):
    """
...
def callSolve(Ox, Oy, Px, Py, theta):
    """
    计算旋转超椭圆边界点。
    Ox, Oy: 障碍物中心坐标
    Px, Py: 机器人位置坐标
    ...
    # 4. 定义超椭圆方程
    def func(x):
        a, b, n = 0.5, 1.08, 4  # 可根据障碍物形状调整
        lx, ly = local_position[0]*x, local_position[1]*x
        return (lx/a)**n + (ly/b)**n - 1

    # 5. 求解root
    # 初值0.5需要根据情况调整，若求解困难可尝试不同初值
...
def si_barrier_cert(dxi, x, obstacles, barrier_gain=90, safety_radius=0.4, magnitude_limit=0.5):
    """
    单积分机器人屏障证书
    dxi: 初始速度输入 (2xN)
    x:   机器人位置 (2xN)
    obstacles: 每行 [Ox, Oy, theta] 表示障碍物中心与旋转角度
    """
    N = dxi.shape[1]
    ...
    for i in range(N):
        ...
        for k in range(obstacles.shape[0]):
            Ox, Oy, theta = obstacles[k, :3]
            # 计算障碍物边界点(全局坐标)
            obstacle_boundary_point = callSolve(Ox, Oy, Px, Py, theta)
            # 相对障碍物中心的向量
    ...
    # 构造QP参数
    # min (dxi^T dxi) 相当于 min ||dxi||^2
    # => H = 2I, f = -2dxi (使得优化在无约束时解为初始dxi)
    N_var = 2*N
    H = 2 * np.eye(N_var)
